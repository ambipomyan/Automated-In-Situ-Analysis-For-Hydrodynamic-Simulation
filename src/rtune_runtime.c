...
int rtune_func_schedule_check(rtune_func_t * func) {
    ...
    for (i=0; i<func->num_vars; i++) {
        ...
        if (start < last_var_end) { //overlapping schedule
            sprintf("The update schedule of var[%d] (%s, %x) overlap with the last var[%d]\n", i, var->stvar.name, var, i-1);
            safe_schedule = 1;
        }
...
static int rtune_objective_collect_vars(rtune_objective_t * obj) {
    ...
    for (i=0; i<obj->num_funcs_input; i++) {
        ...
        for (j=0; j<func->num_vars; j++) {
            ...
            if (k<num_vars) {
                usage_count[k]++;
                printf("var %x(%s) is used more than once (%d times) for th obj %x(%s) via func %x(%s)\n", var, var->stvar.name, usage_count[k], obj, obj->name, func, func->stvar.name);
            } else { // (k==num_vars)
