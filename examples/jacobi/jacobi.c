...
double jacobi_omp(int n, int m, REAL dx, REAL dy, REAL alpha, REAL relax, REAL *u_p, REAL *f_p, REAL tol, int mits);

double jacobi_omp_rtune(int n, int m, REAL dx, REAL dy, REAL alpha, REAL relax, REAL *u_p, REAL *f_p, REAL tol, int mits);
...
int main(int argc, char *argv[]) {
    ...
    jacobi_seq(n, m, dx, dy, alpha, relax, u, f, tol, mits); // jacobi_seq(...)
    elapsed = read_timer_ms() - elapsed;
    printf("seq elapsed time(ms): %12.6g\n", elapsed);
...
#pragma omp single
    ...
    for (i=0; i<num_runs; i++) {
        elapsed += jacobi_omp_rtune(n, m, dx, dy, alpha, relax, uomp, f, tol, mits); // jacobi_omp_rtune(...)
    }
    elapsed = elapsed / num_runs;

    printf("OpenMP (%d threads) elapsed time(ms): %12.6g\n", num_threads, elapsed);
    mflops = (0.001 * mits * (n - 2) * (m - 2) * 13) / elapsed;
    printf("MFLOPS: %12.6g\n", mflops);

    error_check(n, m, alpha, dx, dy, uomp, f);
...
void jacobi_seq(int n, int m, REAL dx, REAL dy, REAL alpha, REAL relax, REAL *u_p, REAL *f_p, REAL tol, int mits) {
    ...
    while ((k <= mits) && (error > tol)) {
        ...
        /* Error check */
        if (k % 50 == 0) printf("Finished %d iteration with error: %g\n", k, error);
        error = sqrt(error) / (n * m);
...
#pragma omp parallel for private(j,i, resid) reduction(+:error)
        ...
        if (omp_get_num_threads() == 1 && k % 50 == 0) printf("Finished %d iteration with error: %g\n", k, error);
        error = sqrt(error) / (n * m);
...
double jacobi_omp_rtune(int n, int m, REAL dx, REAL dy, REAL alpha, REAL relax, REAL *u_p, REAL *f_p, REAL tol, int mits) {
    ...
    short total_num_states = 12;

    short max_num_threads  = 12;
    short min_num_threads  =  1;
    short step_num_threads = -1;
    /* end init tuning parameters */

    /* init tuning models */
    rtune_var_t * num_threads = rtune_var_add_range(jacobi_region, "num_threads", total_num_states, RTUNE_short, &max_num_threads, &min_num_threads, &step_num_threads);
    ...
    /* end init tuning models*/

    double elapsed = read_timer_ms();
    while ((k <= mits)) {
        /* tunign region bound 1 */
        rtune_region_begin(jacobi_region); // can include error = 0.0
...
#pragma omp parallel for private(j,i, resid) reduction(+:error)
        ...
        /* tuning region bound 2 */
        rtune_region_end(jacobi_region); // can include k = k + 1
    }
    elapsed = read_timer_ms() - elapsed;
...
double jacobi_omp_rtune(int n, int m, REAL dx, REAL dy, REAL alpha, REAL relax, REAL *u_p, REAL *f_p, REAL tol, int mits) {
    ...
    rtune_var_set_update_schedule_attr(num_threads, RTUNE_UPDATE_REGION_BEGIN, RTUNE_UPDATE_LIST_SERIES, 0, 20, 0);
    //rtune_var_set_update_schedule_attr(num_threads2, RTUNE_UPDATE_REGION_BEGIN, RTUNE_UPDATE_LIST_SERIES, 70, 5, 6);
    //rtune_func_t * exe_time = rtune_func_add_model(jacobi_region, RTUNE_FUNC_EXT_DIFF, "exe_time", RTUNE_double, read_timer_ms, NULL, 2, num_threads, num_threads2);
    rtune_func_t * exe_time = rtune_func_add_model(jacobi_region, RTUNE_FUNC_EXT_DIFF, "exe_time", RTUNE_double, read_timer_ms, NULL, 1, num_threads);
    rtune_func_set_update_schedule_attr(exe_time, RTUNE_UPDATE_REGION_BEGIN_END_DIFF, RTUNE_UPDATE_BATCH_ACCUMULATE, RTUNE_DEFAULT_NONE, RTUNE_DEFAULT_NONE, RTUNE_DEFAULT_NONE );
    rtune_objective_t * min_exe_time = rtune_objective_add_min(jacobi_region, "min exe time", exe_time);
    rtune_objective_set_search_strategy(min_exe_time, RTUNE_OBJECTIVE_SEARCH_UNIMODAL_ON_THE_FLY);
	...
	rtune_region_begin(jacobi_region);
        //printf("===================== iteration %d ===========================\n", k);
#pragma omp parallel for private(j,i) shared(m,n,uold,u)
        for (i = 0; i < n; i++) {
            for (j = 0; j < m; j++) {
...
#pragma omp parallel for private(j,i, resid) reduction(+:error)
        ...
        //if (k % 50 == 0) printf("Finished %d iteration with error: %g\n", k, error);
        error = sqrt(error) / (n * m);
        rtune_region_end(jacobi_region);
