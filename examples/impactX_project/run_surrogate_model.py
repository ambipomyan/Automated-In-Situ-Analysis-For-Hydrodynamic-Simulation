...
# surrogate model
my_data_arr_size = (5, npart)
my_data_arr = np.zeros(my_data_arr_size)

my_coeff = np.random.randn(4)
my_lr = 0.1
my_n_sample = 100

def obtain_my_data_arr(curr_data_arr):
    for i in range(4):
        my_data_arr[4-i] = my_data_arr[3-i]
    my_data_arr[0] = curr_data_arr[:,0]

    return my_data_arr

def train_my_surrogate_model(my_data_arr, my_lr, my_n_sample):
    global my_coeff
    
    samples = np.random.randint(100000, size=my_n_sample)
    
    epochs = 1000
    X = np.array(my_data_arr[1:, samples]).T
    y = np.array(my_data_arr[0 , samples]).T
    ...
    for epoch in range(epochs):
        for i in range(len(X)):
            pred = np.dot(X[i], my_coeff)
            err  = pred - y[i]
            my_coeff[0] -= my_lr * X[i,0] * err
            my_coeff[1] -= my_lr * X[i,1] * err
            my_coeff[2] -= my_lr * X[i,2] * err
            my_coeff[3] -= my_lr * X[i,3] * err
            loss += err**2
        loss /= my_n_sample
        #print("training_loss:", loss)

def infer_with_my_surrogate_model(my_data_arr, data_arr):
    global my_coeff
    global npart
    
    X = np.array(my_data_arr[1:, :]).T
    y = np.array(my_data_arr[0 , :]).T
    
    preds = np.dot(X, my_coeff)
    # LOSS = MSE
    loss = np.mean((preds - y)**2)
    print("inference loss:", loss)
    
    final_stat_arr = data_arr
    final_stat_arr[:,0] = preds

    return final_stat_arr
...
class DataCollectionSteps(elements.Programmable):
    def __init__(self, stage_i, surrogate_length, stage_start):
    ...
    def surrogate_push(self, pc, step, period):
        ...
        ref_part_x  = np.array(ref_part.x)
        ref_part_y  = np.array(ref_part.y)
        ref_z_i_LPA = np.array(ref_z_i_LPA)
        ref_part_px = np.array(ref_part.px)
        ref_part_py = np.array(ref_part.py)
        ref_part_pz = np.array(ref_part.pz)
        
        ref_part_arr = np.array(
            [
                ref_part_x,
                ref_part_y,
                ref_z_i_LPA,
        ...
        ref_beta_gamma = np.sqrt(np.sum(ref_part.px ** 2))
        
        ref_part_model_final = ref_part_arr

        ref_uz_f = ref_part_model_final[5]
        ref_beta_gamma_final = ref_uz_f
        ref_part_final = np.array(
            [0, 0, ref_z_f, 0, 0, ref_uz_f], dtype=np.float64
        )
        ...
        for lvl in range(pc.finest_level + 1):
            for pti in ImpactXParIter(pc, level=lvl):
                ...
                # create stacks
                data_arr = np.stack([x, y, t, px, py, pt], axis=1).astype(np.float64)
                ...
                my_data_arr = obtain_my_data_arr(data_arr) # (1+4)xnpart, global

                if step <= 64:
                    train_my_surrogate_model(my_data_arr, my_lr, my_n_sample)
                else:
                    data_arr_post_model = infer_with_my_surrogate_model(my_data_arr, data_arr) # npartx6, local

                    # print 2
                    #print("x2", data_arr_post_model[:, 0])
                    #print("y2", data_arr_post_model[:, 1])
...
for i in range(N_stage):
    ...
    lpa = DataCollectionSteps(i, L_surrogate, L_stage_period * i)
    lpa.nslice = n_slice
    lpa.ds = L_surrogate
